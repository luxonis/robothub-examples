...
class ReconstructQrDetections(host_node.BaseNode):
    ...
    def __init__(self, input_node: host_node.BaseNode):
        ...
        self._FRAME_WIDTH = rh.CONFIGURATION["merged_image_size"]
        self._FRAME_HEIGHT = rh.CONFIGURATION["merged_image_size"]
        self._OVERLAP = 450  # not overlap but crop size - overlap
    ...
    def _update_coord_offset(self):
        self._x_offset = int(self._OVERLAP * (self._crop_count // 3))
        self._y_offset = int(self._OVERLAP * (self._crop_count % 3))
