...
class Application(robothub_core.RobotHubApplication):
    ...
    def start_execution(self):
        print(f'Connecting to assigned devices...')
        ...
        print('App has successfully finished execution, you may uninstall it.')
    ...
    def update_bl(self, mxid, name):
        ...
        while time.time() < give_up_time and self.running:
            try:
                ...
                print(f"Could not connect to device {name} with error \'{e}\', retrying...")
                self.wait(0.1)
                continue
        # check if the device was connected and whether its PoE
        if self.running:
            if connected:
                print(f'Connected device with productName "{name}".')
                ...
                if bootloader_type == dai.DeviceBootloader.Type.NETWORK:
                    ...
                    print(f'Device bootloader is type {bootloader_type} which is not type "NETWORK" or "USB". '
                              f'If device is PoE-type and you can\'t update the bootloader, '
                              f'please contact customer support')
                    poe = False

                if poe:
                    print(f'Device is PoE-type, installing latest bootloader version. '
                    ...
                    print(f'Successfully installed latest firmware on the device!')
                    self.bootloader.close()
                else:
                    print(f'Device is NOT PoE-type, skipping...')
            else:
                print(f'Device "{name}" could not be connected within 20s timeout. '
    ...
    def on_stop(self):
        ...
        if self.bootloader:
            try:
                ...
                print(f'Could not close device with error: {e}')
        print('App Stopped')
